...
static int deliverextMessage(MQTTClient* c, EXTED_CMD cmd, int status,
...
void ICACHE_FLASH_ATTR MQTTClientInit(MQTTClient* c, Network* network,
...
static int readPacket(MQTTClient* c, Timer* timer) {
	...
	if (c->ipstack->mqttread(c->ipstack, c->readbuf, 1, TimerLeftMS(timer))
			!= 1)
		goto exit;
...
//    if (packet_type != 65535)
	...
	case CONNACK: {
		...
		if (MQTTDeserialize_connack(&sessionPresent, &connack_rc, c->readbuf,
				c->readbuf_size) == 1)
			rc = connack_rc;
		else
			rc = FAILURE;
	...
	case PUBREC: {
		...
		if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf,
				c->readbuf_size) != 1)
			rc = FAILURE;
		else if ((len = MQTTSerialize_ack(c->buf, c->buf_size, PUBREL, 0,
				mypacketid)) <= 0)
			rc = FAILURE;
	...
	case PUBREL: {
		...
		if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf,
				c->readbuf_size) != 1)
			rc = FAILURE;
		else if ((len = MQTTSerialize_ack(c->buf, c->buf_size, PUBCOMP, 0,
				mypacketid)) <= 0)
			rc = FAILURE;
	...
	case EXTCMD: {
		...
		EXTED_CMD cmd;
		int status;
		if (MQTTDeserialize_extendedcmd((unsigned char*) &msg.dup,
				(int*) &msg.qos, (unsigned char*) &msg.retained,
				(uint64_t*) &msg.id, &cmd, &status, (void**) &msg.payload,
				(int*) &msg.payloadlen, c->readbuf, c->readbuf_size) != 1)
			goto exit;
...
int MQTTClient_get_host_v2(char *appkey, char* url) {
	...
	if (buf) {
		...
		char json_data[256];
		sprintf(json_data,
				"{\"a\":\"%s\",\"n\":\"%s\",\"v\":\"%s\",\"o\":\"%s\"}", appkey, /*${networktype}*/
				"1", "v1.0.0", /*${NetworkOperator}*/"1");
...
int MQTTClient_setup_with_appkey_v2(char* appkey, char *deviceid,
		REG_info *info) {
	int rc = FAILURE;
	...
	if (buf) {
		...
		sprintf(json_data, "{\"a\": \"%s\", \"p\":4, \"d\": \"%s\"}", appkey,
				deviceid);
				...
				if (root) {
					...
					if (ret_size >= 4) {
						...
						strcpy(info->username,
								cJSON_GetObjectItem(root, "u")->valuestring);
						strcpy(info->password,
								cJSON_GetObjectItem(root, "p")->valuestring);
						strcpy(info->device_id,
...
int MQTTPublishToAlias(MQTTClient* c, const char* alias, void *payload,
		int payloadlen) {
	int rc = 0;
	/*TODO: buffer size ?? */
...
int MQTTExtendedCmd(MQTTClient* c, EXTED_CMD cmd, void *payload,
		int payload_len, int qos, unsigned char retained) {
	int rc = FAILURE;
	Timer timer;
	int len = 0;
	uint64_t id = 0;
...
int deliverextMessage(MQTTClient* c, EXTED_CMD cmd, int status,
		int ret_string_len, char *ret_string) {
	int i;
	int rc = SUCCESS;
...
int MQTTSetCallBack(MQTTClient *c, messageHandler cb,
...
int MQTTGetAlias(MQTTClient* c, const char *param) {
	int rc = MQTTExtendedCmd(c, GET_ALIAS, (void *) param, strlen(param),
	DEFAULT_QOS, DEFAULT_RETAINED);
	return rc;
}
...
int MQTTPublish2(MQTTClient *c, const char* topicName, void* payload,
		int payloadlen, cJSON *opt) {
	const char *key[PUBLISH2_TLV_MAX_NUM] = { "topic", "payload", "platform",
			"time_to_live", "time_delay", "location", "qos", "apn_json" };
	uint8_t *p;
	...
	*p++ = (uint8_t)(len & 0xff);
	...
	rc = MQTTExtendedCmd(c, PUBLISH2, pub_buf, p - pub_buf, DEFAULT_QOS, DEFAULT_RETAINED);

	free(pub_buf);
	return rc;
}

int MQTTPublish2ToAlias(MQTTClient *c, const char* alias, void* payload,
		int payloadlen, cJSON *opt) {
	char buf[150];
