...
#endif
		xTaskCreate(yunba_mqtt_client_task, "yunba_mqtt_client_task", 384, //configMINIMAL_STACK_SIZE * 7,
				NULL, 2, NULL);
		break;
	}
}
...
void setup_wifi(void) {
	...
	printf("get wifi config, %s, %s, %d\n", sta_config5->ssid,
			sta_config5->password, ret);
	wifi_set_opmode(STATION_MODE);
	if (strlen(sta_config5->ssid) != 0) {
		os_timer_disarm(&client_timer);
		os_timer_setfn(&client_timer, (os_timer_func_t *) check_wifi_conn,
				NULL);
